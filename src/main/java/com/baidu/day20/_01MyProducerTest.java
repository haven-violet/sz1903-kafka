 ...
 * @Description
 * 生产者API操作
 * 本案例就是学习如何使用API来完成kafka数据的生产
 * 首先肯定需要生产者 --> 就是程序入口Producer
 * 数据被生产到哪里? --> Topic
 * 什么样数据? --> 数据类型
 */
public class _01MyProducerTest {
    public static void main(String[] args) throws IOException, ExecutionException, InterruptedException {
        ...
        /**
         * 创建执行入口
         * 首先我们知道kafka中的数据都是有三个部分组成key, value, timestamp
        ...
        //设置我们要发送的topic
        String topic = "spark";
        ProducerRecord<String, String> record = new ProducerRecord<String, String>(topic, "1112323");
        //通过send发送
        /**
         * 此send方法并不会立即将数据发送到kafka集群,而是先发送到缓冲区,该方法便立即返回
         * 返回给调用者producer,该方法是一个异步方法
         * 而缓冲区满了,或者时间到了,就会将send的数据转换成request请求,提交给kafka集群
         */
